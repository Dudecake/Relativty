...
void Relativty::HMDDriver::retrieve_client_vector_packet_threaded() {
	...
	if ((this->sock = socket(AF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET) {
		Relativty::ServerDriver::Log("Thread3: could not create socket: " + std::string(std::strerror(errno)));
		return;
	}
	...
	if (bind(this->sock, (struct sockaddr *)&server, sizeof(server)) == SOCKET_ERROR) {
		Relativty::ServerDriver::Log("Thread3: Bind failed with error code: " + std::string(std::strerror(errno)));
		return;
	}
	...
	if (this->sock_receive == INVALID_SOCKET) {
		Relativty::ServerDriver::Log("Thread3: accept failed with error code: " + std::string(std::strerror(errno)));
		return;
	}
...
Relativty::HMDDriver::HMDDriver(const std::string myserial) : RelativtyDevice(myserial, "akira_") {
	...
	this->m_hidBuffLength = this->m_bIMUpktIsDMP ? 5 : 64;
